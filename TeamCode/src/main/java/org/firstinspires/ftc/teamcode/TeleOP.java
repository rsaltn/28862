    ...
    public void loop() {
        ...
        if (haveRecentTag && autoAim) {
            ...
            if (Math.abs(servo.getPower()) == 1 && !delay) {
                follower.setTeleOpDrive(Math.pow(-gamepad1.left_stick_y,1/3), Math.pow(-gamepad1.left_stick_x,1/3), Range.clip(headingError * TURN_GAIN, -MAX_AUTO_TURN, MAX_AUTO_TURN), false);
            } else {
                follower.setTeleOpDrive(Math.pow(-gamepad1.left_stick_y,1/3), Math.pow(-gamepad1.left_stick_x,1/3), -gamepad1.right_stick_x, false);
            }
        }
        else {
            follower.setTeleOpDrive(Math.pow(-gamepad1.left_stick_y,1/3), Math.pow(-gamepad1.left_stick_x,1/3), -gamepad1.right_stick_x, false);
        ...
        if(gamepad1.left_bumper)
        {
            ...
            else{sorter.servoRR.setPower(-0.014);}
